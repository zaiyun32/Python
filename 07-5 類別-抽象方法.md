...
### 在父親的某個方法中產生一個例外(exception), 那個方法就有了抽象方法的"概念"; 如果兒子不改寫, 雖然兒子可以生成實例, 但那個方法在呼叫時都會發生例外.
...
### 另一個抽象方法完成的方式是引用ABCMeta類別, 讓父親經由metaclass的宣告成為一個ABCMeta的實例. 另外也用@abstractmethod修飾某個父親方法, 讓它變成抽象方法. 如果兒子沒有改寫抽象方法, 編譯程式時就會出錯, 自然也不會生成任何物件.
